...
addNode appModel parentId widgetType=
    let
        counter = appModel.counter + 1
        newId = IdGenerator.create widgetType counter
        newNode = {
            node =  createAnyModel newId widgetType
             , children = []
        }
        childEdge = createAnyModel newId widgetType
        newNodes = appModel.nodes |> Dict.insert newId newNode |> Dict.update parentId (Maybe.map (\p -> { p | children =  childEdge :: p.children}))
...
linkNode appModel parentId nodeId =
    let
        widgetType = Dict.get nodeId appModel.nodes |> Maybe.map .node |> Maybe.map .widgetType |> Maybe.withDefault DocumentWidget
        childEdge = createAnyModel nodeId widgetType
        newNodes = appModel.nodes |> Dict.update parentId (Maybe.map (\p -> { p | children =  childEdge :: p.children}))
    in
