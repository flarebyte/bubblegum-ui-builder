...
import AnyModel exposing (FieldType, WidgetType, FieldInfoType(..), getFieldInfo, getSupportedFieldTypes) 


createField: FieldType -> Html msg
createField fieldType =
    let
        infoType = getFieldInfo fieldType |> .infoType
    in
        case infoType of
            NumberText ->
...
createLabelField fieldType =
    let
        fieldInfo = getFieldInfo fieldType
    in        
        div [ class "field" ]
        [ label [ class "label" ]
...
createDropdownField fieldType =
    let
        fieldInfo = getFieldInfo fieldType
        options = fieldInfo.suggestions |> List.map (\opt -> option [] [ text opt ])
    in        
       div [ class "field" ]
        [ label [ class "label" ]
            [ text fieldInfo.label ]
...
createWidgetForm widgetType =
    let
        supportedFields = getSupportedFieldTypes widgetType
        fields = supportedFields |> List.map createField
    in
       div [] fields 